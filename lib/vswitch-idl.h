...
enum {
    OVSREC_MYTEST_COL_FIRST,
    OVSREC_MYTEST_N_COLUMNS
};

#define ovsrec_mytest_col_first (ovsrec_mytest_columns[OVSREC_MYTEST_COL_FIRST])

extern struct ovsdb_idl_column ovsrec_mytest_columns[OVSREC_MYTEST_N_COLUMNS];

const struct ovsrec_mytest *ovsrec_mytest_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct ovsrec_mytest *ovsrec_mytest_first(const struct ovsdb_idl *);
const struct ovsrec_mytest *ovsrec_mytest_next(const struct ovsrec_mytest *);
#define OVSREC_MYTEST_FOR_EACH(ROW, IDL) \
        for ((ROW) = ovsrec_mytest_first(IDL); \
             (ROW); \
             (ROW) = ovsrec_mytest_next(ROW))
#define OVSREC_MYTEST_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = ovsrec_mytest_first(IDL); \
             (ROW) ? ((NEXT) = ovsrec_mytest_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int ovsrec_mytest_get_seqno(const struct ovsdb_idl *);
unsigned int ovsrec_mytest_row_get_seqno(const struct ovsrec_mytest *row, enum ovsdb_idl_change change);
const struct ovsrec_mytest *ovsrec_mytest_track_get_first(const struct ovsdb_idl *);
const struct ovsrec_mytest *ovsrec_mytest_track_get_next(const struct ovsrec_mytest *);
#define OVSREC_MYTEST_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = ovsrec_mytest_track_get_first(IDL); \
             (ROW); \
             (ROW) = ovsrec_mytest_track_get_next(ROW))

void ovsrec_mytest_init(struct ovsrec_mytest *);
void ovsrec_mytest_delete(const struct ovsrec_mytest *);
struct ovsrec_mytest *ovsrec_mytest_insert(struct ovsdb_idl_txn *);

void ovsrec_mytest_verify_first(const struct ovsrec_mytest *);

const struct ovsdb_datum *ovsrec_mytest_get_first(const struct ovsrec_mytest *, enum ovsdb_atomic_type key_type);

void ovsrec_mytest_set_first(const struct ovsrec_mytest *, const char *first);
...
#define ovsrec_table_mytest (ovsrec_table_classes[OVSREC_TABLE_MYTEST])
