...
static struct ovsrec_mytest *
ovsrec_mytest_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct ovsrec_mytest, header_) : NULL;
}
...
static void
ovsrec_mytest_parse_first(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct ovsrec_mytest *row = ovsrec_mytest_cast(row_);

    ovs_assert(inited);
    if (datum->n >= 1) {
        row->first = datum->keys[0].string;
    } else {
        row->first = "";
    }
}
...
static void
ovsrec_mytest_unparse_first(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
ovsrec_mytest_init__(struct ovsdb_idl_row *row)
{
    ovsrec_mytest_init(ovsrec_mytest_cast(row));
}

/* Clears the contents of 'row' in table "mytest". */
void
ovsrec_mytest_init(struct ovsrec_mytest *row)
{
    memset(row, 0, sizeof *row); 
}

/* Searches table "mytest" in 'idl' for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct ovsrec_mytest *
ovsrec_mytest_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return ovsrec_mytest_cast(ovsdb_idl_get_row_for_uuid(idl, &ovsrec_table_classes[OVSREC_TABLE_MYTEST], uuid));
}

/* Returns a row in table "mytest" in 'idl', or a null pointer if that
 * table is empty.
 *
 * Database tables are internally maintained as hash tables, so adding or
 * removing rows while traversing the same table can cause some rows to be
 * visited twice or not at apply. */
const struct ovsrec_mytest *
ovsrec_mytest_first(const struct ovsdb_idl *idl)
{
    return ovsrec_mytest_cast(ovsdb_idl_first_row(idl, &ovsrec_table_classes[OVSREC_TABLE_MYTEST]));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct ovsrec_mytest *
ovsrec_mytest_next(const struct ovsrec_mytest *row)
{
    return ovsrec_mytest_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int ovsrec_mytest_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_MYTEST]);
}

unsigned int ovsrec_mytest_row_get_seqno(const struct ovsrec_mytest *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct ovsrec_mytest *
ovsrec_mytest_track_get_first(const struct ovsdb_idl *idl)
{
    return ovsrec_mytest_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_MYTEST]));
}

const struct ovsrec_mytest
*ovsrec_mytest_track_get_next(const struct ovsrec_mytest *row)
{
    return ovsrec_mytest_cast(ovsdb_idl_track_get_next(&row->header_));
}


/* Deletes 'row' from table "mytest".  'row' may be freed, so it must not be
 * accessed afterward.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
ovsrec_mytest_delete(const struct ovsrec_mytest *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}

/* Inserts and returns a new row in the table "mytest" in the database
 * with open transaction 'txn'.
 *
 * The new row is assigned a randomly generated provisional UUID.
 * ovsdb-server will assign a different UUID when 'txn' is committed,
 * but the IDL will replace any uses of the provisional UUID in the
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct ovsrec_mytest *
ovsrec_mytest_insert(struct ovsdb_idl_txn *txn)
{
    return ovsrec_mytest_cast(ovsdb_idl_txn_insert(txn, &ovsrec_table_classes[OVSREC_TABLE_MYTEST], NULL));
}

/* Causes the original contents of column "first" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "first" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction commits, then the transaction aborts and ovsdb_idl_txn_commit()
 * returns TXN_AGAIN_WAIT or TXN_AGAIN_NOW (depending on whether the database
 * change has already been received).
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "first" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "first" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     ovsrec_mytest_insert()).
 *
 *   - If "first" has already been modified (with
 *     ovsrec_mytest_set_first()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * ovsrec_mytest_set_first() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
ovsrec_mytest_verify_first(const struct ovsrec_mytest *row)
{
    ovs_assert(inited);
    ovsdb_idl_txn_verify(&row->header_, &ovsrec_mytest_columns[OVSREC_MYTEST_COL_FIRST]);
}

/* Returns the "first" column's value from the "mytest" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes first's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "first" member in ovsrec_mytest. */
const struct ovsdb_datum *
ovsrec_mytest_get_first(const struct ovsrec_mytest *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &ovsrec_mytest_col_first);
}

/* Sets the "first" column from the "mytest" table in 'row' to
 * 'first'.
 *
 * The caller retains ownership of the arguments. */
void
ovsrec_mytest_set_first(const struct ovsrec_mytest *row, const char *first)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.n = 1;
    datum.keys = &key;
    key.string = CONST_CAST(char *, first);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &ovsrec_mytest_columns[OVSREC_MYTEST_COL_FIRST], &datum);
}

struct ovsdb_idl_column ovsrec_mytest_columns[OVSREC_MYTEST_N_COLUMNS];

static void
ovsrec_mytest_columns_init(void)
{
    struct ovsdb_idl_column *c;

    /* Initialize ovsrec_mytest_col_first. */
    c = &ovsrec_mytest_col_first;
    c->name = "first";
    ovsdb_base_type_init(&c->type.key, OVSDB_TYPE_STRING);
    c->type.key.u.string.minLen = 0;
    ovsdb_base_type_init(&c->type.value, OVSDB_TYPE_VOID);
    ...
    c->type.n_max = 1;
    c->mutable = true;
    c->parse = ovsrec_mytest_parse_first;
    c->unparse = ovsrec_mytest_unparse_first;
}

/* sFlow table. */
...
struct ovsdb_idl_table_class ovsrec_table_classes[OVSREC_N_TABLES] = {
    ...
    {"mytest", false,
     ovsrec_mytest_columns, ARRAY_SIZE(ovsrec_mytest_columns),
     sizeof(struct ovsrec_mytest), ovsrec_mytest_init__},
    {"sFlow", false,
